...
-- LSP keymaps
map("n", "<leader>rn", lsp_buf.rename, { desc = "[R]e[n]ame" })
map("n", "<leader>ca", lsp_buf.code_action, { desc = "[C]ode [A]ction" })
map("n", "<leader>ih", function()
  vim.lsp.inlay_hint.enable(not vim.lsp.inlay_hint.is_enabled())
end, { desc = "[I]nlay [H]int Toggle" })

-- Signature help and hover
map("n", "K", lsp_buf.hover, { desc = "Hover Documentation" })
map({ "n", "i", "s" }, "<C-k>", lsp_buf.signature_help, { desc = "Signature Documentation", silent = true })

-- Lesser used LSP functionality
map("n", "<leader>wa", lsp_buf.add_workspace_folder, { desc = "[W]orkspace [A]dd Folder" })
map("n", "<leader>wr", lsp_buf.remove_workspace_folder, { desc = "[W]orkspace [R]emove Folder" })
